...
@section('content')
                    ...
                    <div class="d-flex align-items-center gap-2 gap-lg-3">
                        ...
                        <div class="m-0">
                            ...
                            <div class="menu menu-sub menu-sub-dropdown w-md-1000px" data-kt-menu="true"
                                ...
                                <!--begin::Form-->
                                <div class="px-7 py-5">
                                    <form action="{{ url('esh2/branchs/list') }}" method="post" class="row">
                                        @csrf
    ...
    @endif-->
            ...
            <div class="align-items-center gap-2 gap-lg-3 p-3"
                ...
                <form action="{{ url('esh2/branchs/list') }}" method="post" class="align-content-center">
                    @csrf
                                            ...
                                            @foreach ($data as $enq)
                                                <tr data-id="{{ $enq->id ?? '' }}" class="enquiryTr">
                                                    ...
                                                    <td class="pt-0 d-flex">
                                                        ...
                                                        <!-- Edit Button -->
                                                        <a target="_blank" href="{{ url('esh2/branchs/edit') }}/{{ $enq->id }}" 
                                                        ...
                                                        <!-- Delete Button -->
                                                        <form action="{{ url('esh2/branchs/delete/'.$enq->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this expenses?');">
                                                            @csrf
                                                            @method('DELETE')
                                                    ...
                                                    <td class="pt-0">{{ $i++ }}</td>
                                                    <td class="pt-0"><small>{{ $enq->branch_name ?? '' }}</small></td>
                                                    <td class="pt-0"><small>{{ $enq->owner_name ?? '' }}</small></td>
                                                    <td class="pt-0"><small>{{ $enq->branch_code ?? '' }}</small></td>
                                                    <td class="pt-0"><small>{{ $enq->gst_no ?? '' }}</small></td>
                                                    <td>
                                                        @if ($enq->status == 1)
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            ...
                            url: "{{ url('esh2/branchs/statusChange') }}",
                            type: "POST",
                            data: formData,
                            success: function(response) {
                                if (response.success) {
