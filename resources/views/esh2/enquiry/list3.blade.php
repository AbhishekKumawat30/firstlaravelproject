                ...
                <div class="d-flex">
                    ...
                    @foreach($label as $labelsearch)
                        <div class="form-check form-check-custom form-check-solid">
                            <input type="submit" class="form-check-input pointer" name="labelsearch" type="checkbox" value="{{ $labelsearch->id ?? '' }}" style="border-radius:50%; background-color : {{ $labelsearch->color ?? '' }}"/>
                        </div> &nbsp; &nbsp; &nbsp; 
                    @endforeach
                    @endif
                </div>
    ...
    <div class="card-body py-4">
        ...
        <div id="kt_table_users_wrapper" class="dt-container dt-bootstrap5 dt-empty-footer">
            <div id="" class="table-responsive">
                <table class="table table-hover table-striped align-middle table-row-dashed fs-7 dataTable" id="kt_table_enquiry" style="width: 1199.5px;"></table>
            </div>
        </div>
                    ...
                    >
                        ...
                        <div class="fv-row mb-7 col-md-6 d-flex">
                            ...
                            @foreach($label as $lab)
                                <div class="form-check form-check-custom form-check-solid">
                                    <input class="form-check-input pointer" name="label[]" type="checkbox" value="{{ $lab->id ?? '' }}" style="border-radius:50%; background-color : {{ $lab->color ?? '' }}"/>
                                </div> &nbsp; &nbsp; &nbsp; 
                            @endforeach
                            @endif
                        </div>
...
$(document).ready(function(){
    ...
    var enquiryStatus = @json($enquiryStatus);
    var labelTable = @json($label);
    var session_id = @json(Session::get('id'));
    var enquiryth = `<thead>
        <tr class="text-start text-muted fw-bold fs-7 text-uppercase gs-0">
            <th class="w-10px">Actions</th>
    ...
    </thead><tbody class="text-gray-600 fw-semibold" id="tableBody">`;
    var enquiryRaw = '';
    var i = 0;
    ...
    $(enquiry).each(function(index, value){
        ...
        if(value.department_id == 1){
            var department = `<span class="badge badge-light-success">School</span>`;
        }else if(value.department_id == 2){
            var department = `<span class="badge badge-light-info">Coaching</span>`;
        }else if(value.department_id == 3){
            var department = `<span class="badge badge-light-warning">Library</span>`;
        }else if(value.department_id == 4){
            var department = `<span class="badge badge-light-danger">Hostel</span>`;
        }else if(value.department_id == 5){
            var department = `<span class="badge badge-light-primary">Website</span>`;
        }else if(value.department_id == 6){
            var department = `<span class="badge badge-light-secondary">CRM</span>`;
        }else if(value.department_id == 7){
            var department = `<span class="badge badge-light-dark">Custom</span>`;
        }else{
            var department = '';
        }
    ...
    enquiryRaw += `
                    @php
                        $enqStatus = $enquiryStatus->where('id', ` + value.responce_status_id + `)->first();
                        $user = DB::table('users')->where('id', ` + value.user_id + `)->whereNull('deleted_at')->get('name')->first();
                    @endphp
                <tr>
                    <td>
                        <form action="{{ url('esh2/enquiry/list') }}" method="post">
                    ...
                    <td class="pt-0">` + i + `</td>
                    <td class="pt-0 d-flex align-items-center"><div class="symbol symbol-circle symbol-50px overflow-hidden me-3"><a class="text-dark" target="blank" href="https://web.rusoft.in/esh2/enquiry/view/` + value.id + `">
                        @if(!empty(` + value.image + `))
                            <div class="symbol-label"><img src="{{ env('IMAGE_SHOW_PATH') . 'esh2/enquiry/' . ` + value.image + ` }}" alt="Enquiry Image" class="w-100" /></div>
                        @else
                            <div class="symbol-label fs-3 bg-light-danger text-danger">{{ $enq->name[0] ?? '' }}</div>
                        @endif
                        </a></div><div class="d-flex flex-column"><small>` + value.name + `</small>` + department + ` </div></td>
                    <td class="pt-0"><small><a href="tel://` + value.mobile + `">` + value.mobile + `</a> <br><a href="tel://` + value.mobile2 + `">` + value.mobile2 + `</a> &nbsp; <a href="mailto:` + value.email + `">` + value.email + `</a></small></td>
                    <td class="pt-0"><small>` + value.address + `</small></td>
                    <td class="pt-0 text-gray-800"><small>` + value.reminder_date + `<hr class="m-0"><span class="text-primary">` + value.message + `</span></small></td>
                    <td class="pt-0"><small><div class="badge badge-light-{{ $enqStatus->color ?? '' }} fw-bold">{{ $enqStatus->name ?? '' }}</div><br>{{ $user->name ?? '' }} | {{ date('l d/m/Y h:i a', strtotime(` + value.updated_at + `)) ?? '' }}</small></td>
                </tr>
    `
        
    });
    var appendTable = enquiryth + enquiryRaw + `</tbody>`;
        $('#kt_table_enquiry').html(appendTable);
})
</script>        
