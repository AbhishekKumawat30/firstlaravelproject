    ...
    <div class="d-flex flex-column flex-column-fluid">
        ...
        <!--end::Menu 1-->        </div>
            ...
            <!--begin::Primary button-->
                     <a href="{{ url('esh2/customers/gettingStarted') }}" class="d-flex btn btn-sm fw-bold btn-primary" >
                    ...
                    @if(!empty($data))
                        ...
                        <tr data-id="{{ $enq->id ?? '' }}" class="enquiryTr">
                            ...
                            <td class="pt-0 d-flex">  
                                ...
                                <a target="_blank" href="{{ url('esh2/customers/edit') }}/{{ $enq->id }}" class="btn btn btn-icon btn-info me-2 mb-2 btn-outline btn-outline-dashed btn-outline-info pulse pulse-info "><i class="ki-duotone ki-pencil fs-7"><span class="path1"></span><span class="path2"></span></i><span class="pulse-ring"></span></a>
                            </td>
                            <td class="pt-0">{{ $i++ }}</td>
                            <td>
                                <div class="symbol symbol-circle symbol-50px overflow-hidden me-3"><a class="text-dark" target="blank" href="https://web.rusoft.in/esh2/enquiry/view/{{ $enq->id ?? '' }}">
                                @if(!empty($enq->image))
                                    <div class="symbol-label"><img src="{{ env('IMAGE_SHOW_PATH') . 'esh2/customers/' . $enq->image }}" alt="Enquiry Image" class="w-100" /></div>
                                @else
                            ...
                            <td class="pt-0 align-items-center">
                                  ...
                                  {{ $enq->name ?? '' }} <br> <small> <span class="text-primary" >  Reg. Date: {{ date('d-M-Y', strtotime($enq->created_at)) }} </span> <span class="text-success" >Email: {{ $enq->email ?? '' }}</span> <br>  {{ $enq->address ?? '' }} </small>
                               
                            </td>
                            <td class="pt-0"><small><a href="tel://{{ $enq->mobile ?? '' }}">{{ $enq->mobile ?? '' }}</a> <br> </small>
                            <small><a href="tel://{{ $enq->mobile2 ?? '' }}">{{ $enq->mobile2 ?? '' }}</a></td>
                            <td class="pt-0"><small>{{ $enq->project_name ?? '' }}</small></td>
                            <td class="pt-0"><small>{{ $enq->emc_date ?? '' }}</small></td>
                            <td class="pt-0"><small>{{ $enq->amc_amount ?? '' }}</small></td>
                            <td class="pt-0"><small>{{ $enq->remark ?? '' }}</small></td>
                           </tr>
                        @endforeach
                    @endif
                    </tbody>                    
