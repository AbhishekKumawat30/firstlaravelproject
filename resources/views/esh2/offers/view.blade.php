...
@section('content')
    ...
    @endif-->
                        ...
                        <a href="{{ url('esh2/offers/add') }}" class="d-flex btn btn-sm fw-bold btn-primary">
                                            ...
                                            @foreach ($data as $enq)
                                                <tr data-id="{{ $enq->id ?? '' }}" class="enquiryTr">
                                                    ...
                                                    <td class="pt-0 d-flex">
                                                        ...
                                                        <!-- Edit Button -->
                                                        <a target="_blank" href="{{ url('esh2/offers/edit') }}/{{ $enq->id }}" 
                                                        ...
                                                        <!-- Delete Button -->
                                                        <form action="{{ url('esh2/offers/delete/'.$enq->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this Offer?');">
                                                            @csrf
                                                            @method('DELETE')
                                                    ...
                                                    <td class="pt-0">{{ $i++ }}</td>
                                                    <td class="pt-0"><small>{{ $enq->name ?? '' }}</small></td>
                                                    <td class="pt-0"><small>{{ $enq->from_date ?? '' }}</small></td>
                                                    <td class="pt-0"><small>{{ $enq->to_date ?? '' }}</small></td>
                                                    <td class="pt-0"><small>{{ $enq->promo_code ?? '' }}</small></td>
                                                    <td>
                                                        @if ($enq->status == 1)
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            ...
                            url: "{{ url('esh2/offers/statusChange') }}",
                            type: "POST",
                            data: formData,
                            success: function(response) {
                                if (response.success) {
